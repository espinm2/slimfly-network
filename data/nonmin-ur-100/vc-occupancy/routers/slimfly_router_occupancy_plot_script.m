...
ylabel('Router ID');
zlabel('Occupancy');
title('Router VC0 Occupancy vs Time')
colorbar;
caxis([0, 100])
...
ylabel('Router ID');
zlabel('Occupancy');
title('Router VC1 Occupancy vs Time')
colorbar;
caxis([0, 100])
axis([0 100 0 TOTAL_ROUTERS*(RADIX) 0 100])
...
Z = zeros(length(X),length(Y));
for j=1:1:TOTAL_ROUTERS
    for i=3:VC_SIZE:RADIX*VC_SIZE     %starting at i=1 gives all vc0, 2->vc1, 3->vc2, 4->vc3
        Z(:,indx) = Data(j,:,i);
        indx = indx+1;
    end
end
...
ylabel('Router ID');
zlabel('Occupancy');
title('Router VC2 Occupancy vs Time')
colorbar;
caxis([0, 100])
axis([0 100 0 TOTAL_ROUTERS*(RADIX) 0 100])
...
Z = zeros(length(X),length(Y));
for j=1:1:TOTAL_ROUTERS
    for i=4:VC_SIZE:RADIX*VC_SIZE     %starting at i=1 gives all vc0, 2->vc1, 3->vc2, 4->vc3
        Z(:,indx) = Data(j,:,i);
        indx = indx+1;
    end
end
...
ylabel('Router ID');
zlabel('Occupancy');
title('Router VC3 Occupancy vs Time')
colorbar;
caxis([0, 100])
axis([0 100 0 TOTAL_ROUTERS*(RADIX) 0 100])
